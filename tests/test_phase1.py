...
try:
    from backend.rules import load_rules, get_rules_for_claude
    rules = load_rules()
...
try:
    ...
    test("API server running", False,
         "→ Is uvicorn running? Start it first!")
...
try:
    ...
    r = requests.post(
        f"{BASE_URL}/ask",
        json={"question": "Show me the top 5 most delayed flights"}
    )
    ...
    r3 = requests.post(
        f"{BASE_URL}/query",
        json={"query": "SELECT airline, delay FROM airlines WHERE delay > 0 ORDER BY delay DESC LIMIT 5"}
    )
